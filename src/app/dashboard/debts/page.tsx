...
import { listDebts, recordDebtPayment } from '@/lib/api/finance'
import Sidebar from '@/components/dashboard/Sidebar'
import DashboardHeader from '@/components/dashboard/DashboardHeader'
import { Plus, Search, AlertTriangle, ChevronDown, DollarSign, TrendingDown, TrendingUp, Loader2, X } from 'lucide-react'
...
export default function DebtsPage() {
  ...
  const filtered = useMemo(() => {
    if (!search.trim()) return allDebts
    const q = search.toLowerCase()
    return allDebts.filter(d => partyName(d).toLowerCase().includes(q) || d.description.toLowerCase().includes(q))
  }, [allDebts, search])

  const totalReceivable = allDebts.filter(d => d.direction === 'receivable' && d.status !== 'settled').reduce((s, d) => s + d.amount_due, 0)
  const totalPayable    = allDebts.filter(d => d.direction === 'payable'    && d.status !== 'settled').reduce((s, d) => s + d.amount_due, 0)
    ...
    <div className="min-h-screen bg-background text-white">
      ...
      <div className="lg:pl-72">
        ...
        <main className="p-6 lg:p-10 space-y-8 pb-20">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <MiniStat label="Overdue"       value={String(overdueCount)} color={overdueCount > 0 ? 'text-red-400' : 'text-gray-500'} sub="need attention" />
          </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {DIR_FILTERS.map(d => (
                <button key={d} onClick={() => setDir(d)}
                  ...
                  {d || 'All'}{d === 'receivable' ? ' (owed to you)' : d === 'payable' ? ' (you owe)' : ''}
                </button>
              ))}
            </div>
            <div className="flex gap-2 flex-wrap">
          ...
          <div className="bg-[rgba(255,255,255,0.03)] border border-white/10 rounded-2xl overflow-hidden">
            {isLoading ? (
              <div className="p-10 text-center text-gray-500 text-sm">Loading debts...</div>
            ) : filtered.length === 0 ? (
              ...
              <div className="divide-y divide-white/5">
                {filtered.map(debt => {
                  ...
                  return (
                    <div key={debt.id} onClick={() => setSelected(debt)}
                      ...
                      <div className="hidden lg:block text-right shrink-0">
                        {debt.due_date ? (
                          <p className={cn('text-xs font-medium', days !== null && days < 0 ? 'text-red-400' : days !== null && days <= 7 ? 'text-yellow-400' : 'text-gray-400')}>
                            {days !== null && days < 0 ? `${Math.abs(days)}d overdue` : days !== null ? `Due in ${days}d` : fmtDate(debt.due_date)}
                          </p>
                        ) : (
      ...
      {selected && (
        <DebtDrawer debt={selected} onClose={() => setSelected(null)}
          onPayment={() => { qc.invalidateQueries({ queryKey: ['debts'] }); setSelected(null) }} />
      )}
    </div>
  )
}
