...
function StatCard({ icon: Icon, label, value, sub, color }: {
  ...
  return (
    <div className="bg-dash-surface border border-dash-border rounded-2xl p-5 flex items-start gap-4">
      <div className={cn('w-11 h-11 rounded-xl border flex items-center justify-center shrink-0', color)}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-2xl font-bold text-foreground tracking-tight leading-none truncate">
          {value}
        </p>
...
export default function SalesPage() {
  ...
  return (
    <div className="min-h-screen bg-dash-bg text-foreground">
                    ...
                    <div key={sale.id} className="group hover:bg-dash-hover/50 transition-colors">
                      ...
                      <div className="hidden sm:grid grid-cols-[1fr_1fr_130px_150px_110px_48px] gap-4 items-center px-6 py-4">
                        ...
                        <button className="text-left min-w-0" onClick={() => setSelectedId(sale.id)}>
                          ...
                          {sale.staff_name && (
                            <p className="text-xs text-muted-foreground/60 truncate mt-0.5">👤 {sale.staff_name}</p>
                          )}
                        </button>
      ...
      {/* Modals */}
      <RecordSaleModal open={showModal} onOpenChange={setShowModal} onSuccess={refetch} />
      <SaleDetailDrawer
        saleId={selectedId}
