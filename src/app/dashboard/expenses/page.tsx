...
import { listExpenses, deleteExpense } from '@/lib/api/finance'
...
export default function ExpensesPage() {
  ...
  const stats = useMemo(() => {
    ...
    for (const exp of filtered) {
      ...
      if (exp.expense_type === 'opex') totalOpex += amount
      if (exp.expense_type === 'capex') totalCapex += amount
      ...
      <div className="lg:pl-72">
        ...
        <main className="p-6 lg:p-10 space-y-8 pb-20">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <MiniStat
              label="OpEx"
              value={fmt(stats.totalOpex)}
              color="text-blue-400"
              sub="operating expenses"
            />
            <MiniStat
              label="CapEx"
              value={fmt(stats.totalCapex)}
              color="text-purple-400"
              sub="capital expenses"
            />
            <MiniStat
              label="Top Category"
              value={stats.topCategoryEntry ? stats.topCategoryEntry[0] : '—'}
              color="text-yellow-400"
              sub={stats.topCategoryEntry ? fmt(stats.topCategoryEntry[1]) : 'No data yet'}
            />
          </div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span
                          className={cn(
                            ...
                            TYPE_CLS[exp.expense_type] ?? TYPE_CLS.opex
                          )}
                        >
                          {exp.expense_type}
                        </span>
